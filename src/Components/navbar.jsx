            ...
            {ListNavbar.map((item, index) => (
              <li key={index} className=" group block  relative  ">
                <Link to={item.link} className="uppercase block overflow-hidden">
                  <motion.div animate={transparant ? { color: "#000000" } : { color: "#ffffff" }} transition={{ duration: 0.1, ease: "easeInOut" }} className="flex text-xs lg:text-sm  items-center gap-0.5">
                  ...
                  <span
                    className={`w-full group-hover:h-1 block transition-all duration-300 group-hover:opacity-100 group-hover:translate-0 ${
                      (pathname === "/" && item.link === "/") || (item.link !== "/" && pathname.includes(item.link)) ? "opacity-100 translate-0 h-1" : "opacity-0 -translate-x-full h-0"
            ...
            {ListNavbar.map((item, index) => (
              <li key={index} className="block pb-3">
                {item.hasOwnProperty("list_link") ? (
                  <section className="w-full overflow-hidden">
                    ...
                    <section className={`w-full  pt-3 transition-all origin-top  duration-300 ${openListNavbarMobile[index] ? "h-auto scale-y-100 opacity-100 " : "h-0 scale-y-0  opacity-0 "} `}>
                      <ul className="space-y-3 pl-2">
                        {item.list_link.map((item, index) => (
                          <li key={index}>
                            <Link to={item.link} onClick={() => setIsOpen(false)} className="block  text-black hover:text-blue-400 duration-300 ">
                              {item.name}
                            </Link>
                          </li>
                        ))}
                  ...
                  <Link to={item.link} onClick={() => setIsOpen(false)} className="flex justify-between items-center">
                    {item.name}{" "}
                  </Link>
                )}
              </li>
